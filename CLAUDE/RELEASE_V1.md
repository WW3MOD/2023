...
- [ ] **Allied shared vision blinks rapidly (~3-4 Hz) before stopping** — *Reported 260505*: user observed shared vision from an allied player flicker on/off ~3-4 times per second for several seconds, in an area with allied vehicles, then stop and return to normal. User suspects one of the vehicles was being destroyed at the time. **Hypotheses to investigate:** (a) `Vision` trait is `ConditionalTrait` — if some condition gating vision toggles at high frequency (e.g. damage-state condition crossing a threshold from incoming hits) the trait disable/enable triggers `UpdateCells` (`AffectsMapLayer.Tick`). (b) `^StandardVisionWhenLoaded` requires `loaded` condition — a transport rapidly loading/unloading would blink vision. (c) `IVisionModifier` returning 100/non-100 based on a flickering condition would change cached range and trigger recompute. (d) `VehicleCrew` ejecting crew one-by-one on critical damage may toggle conditions that gate vision. (e) Garrison dynamic ownership (enter→claim, neutral on empty) flips the actor's owner — if the unit was a garrisoned vehicle this would flicker the share. Need more info from user: which faction / vehicle types, was the vehicle exiting/entering a transport or building, was it the *destroyed* vehicle that lost vision or others around it? helicopter→helicopter missiles fly to the side and silently vanish at the impact point with no explosion, when they should `FlyStraightIfMiss` past the target. Probably target-loss bug or `FlyStraightIfMiss` not gated correctly. `engine/OpenRA.Mods.Common/Projectiles/Missile.cs`. *Reported 260503*
