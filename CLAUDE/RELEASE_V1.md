...
- [ ] **Artillery: force-attack-ground does not fire** — *Reported 260508 via test harness*: User issued a Ctrl+click force-attack on a ground location; Paladin (already engaged on a vehicle) never fired the order. Paladin has `AttackTurreted.ForceFireIgnoresActors: True` and no `RequiresForceFire`. Suspects: (a) order eaten while in setup-ticks countdown, (b) `PersistentTargeting: False` overriding queued force-fire when current target still present, (c) HoldFireWhileMoving / NoSelfDefenseInterrupt path swallowing the order. Needs repro with diagnostic logging.
