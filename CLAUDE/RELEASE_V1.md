...
- [T] **Garrison playtest 260504 — observations checklist** *(reported live during playtest)*
  ...
  - [T] **Supply truck deploy → drop cache as box** — *Fixed 260504*: commit e573530b (Mar 25) replaced TRUK's `DropsCrate` (which provided `IIssueDeployOrder`) with `CargoSupply` and the deploy-order entry point was lost — only the sidebar `DROP_SUPPLY` button worked. Added `IIssueDeployOrder` + `DeployOrderTargeter("DropCargoSupply")` to `CargoSupply`; deploy fires `UnloadCargoSupply` with `ExtraData = (uint)supplyCount` which routes through the existing `DropSupplyCache` path (auto-merges with any existing cache on the cell). `CanIssueDeployOrder` gates on `supplyCount > 0` and cell containing only self or a SUPPLYCACHE. Helicopters with both Cargo+CargoSupply will now also unload supply on deploy alongside passengers — that's an air-drop feature, not a regression. Verify in playtest: select TRUK, press deploy hotkey or right-click on self → SUPPLYCACHE spawns at truck's cell, truck retains 0 supply.
