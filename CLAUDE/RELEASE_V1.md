...
- [T] **Shift+G on attack-ground orders converts them to move orders** — *Fixed 260504 (commit 539d5ea3)*: AttackBase.GetAttackActivity returns 3 different classes (Activities.Attack for AttackFrontal, AttackFollow.AttackActivity for turreted, AttackOmni.SetTarget for omni). Old `is Attack` check only caught the first; the other two fell through to the `Move` fallback. Promoted AttackFollow.AttackActivity to internal access and extended GroupScatterHotkeyLogic to match all three, with TargetLineNodes fallback for the nested classes (they don't override GetTargets).
