...
- [x] **Lobby category headers showed as broken boxes** — fixed 260503 in `engine/OpenRA.Mods.Common/Widgets/Logic/Lobby/LobbyOptionsLogic.cs`. Unicode box-drawing chars `──` replaced with ASCII `--`. The `TinyBold` font has no glyphs for `──` (U+2500).
