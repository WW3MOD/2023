...
| ANY edit to an **autotest scenario** under `tools/autotest/scenarios/` — terrain, `Bounds`, actors or YAML | **Scenarios are not maps to the tooling: the map-shaped gates cover the 10 shipped maps and skip all 273 scenarios**, so their green is byte-identical before and after your edit. Confirmed for **both** `make nav-guard` (baseline is `mods/ww3mod/maps` only; that misread cost two workers on 2026-09-01) and **bare `--check-yaml`/`make test`** (its map list defaults to `MapClassification.System`; scenarios are `Unknown`). Assume a third gate has the same hole until you have read its file list. Lint one with `./utility.sh --check-yaml ../tools/autotest/scenarios/<name>` — **the `../` is required**, the path is relative to `engine/` — and inspect one with `nav_guard.py report --scenarios --map <name>`. Mechanism and what else the targeted form does not catch: [`DOCS/reference/conventions.md`](DOCS/reference/conventions.md) §"Scenarios are NOT maps to the tooling". Connectivity and binding checks that DO apply: [`DOCS/recipes/AUTOTEST.md`](DOCS/recipes/AUTOTEST.md) §"Verify before you ask for a slot" |
