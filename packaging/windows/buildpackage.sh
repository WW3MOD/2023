...
function build_platform()
{
	...
	# The uninstaller deletes by explicit manifest, not by wildcard (a wildcard
	# uninstall would have taken unrelated user files out of a badly-chosen
	# $INSTDIR -- see gen-uninstall-manifest.py). Generate it from the tree we
	# just built, and reuse that same walk for the Add/Remove Programs size so
	# the installer never has to scan $INSTDIR at install time.
	UNINSTALL_MANIFEST="${PACKAGING_DIR}/uninstall-manifest-${PLATFORM}.nsh"
	INSTALL_SIZE_KB=$(python3 "${PACKAGING_DIR}/gen-uninstall-manifest.py" "${BUILTDIR}" "${UNINSTALL_MANIFEST}")

	echo "Building Windows setup.exe (${PLATFORM})"
	pushd "${PACKAGING_DIR}" > /dev/null
	makensis -V2 -DSRCDIR="${BUILTDIR}" -DTAG="${TAG}" -DMOD_ID="${MOD_ID}" -DPACKAGING_WINDOWS_INSTALL_DIR_NAME="${PACKAGING_WINDOWS_INSTALL_DIR_NAME}" -DPACKAGING_WINDOWS_LAUNCHER_NAME="${PACKAGING_WINDOWS_LAUNCHER_NAME}" -DPACKAGING_DISPLAY_NAME="${PACKAGING_DISPLAY_NAME}" -DPACKAGING_WEBSITE_URL="${PACKAGING_WEBSITE_URL}" -DPACKAGING_AUTHORS="${PACKAGING_AUTHORS}" -DPACKAGING_WINDOWS_REGISTRY_KEY="${PACKAGING_WINDOWS_REGISTRY_KEY}" -DPACKAGING_WINDOWS_LICENSE_FILE="${TEMPLATE_ROOT}/${PACKAGING_WINDOWS_LICENSE_FILE}" -DOUTFILE="${OUTPUTDIR}/${PACKAGING_INSTALLER_NAME}-${TAG}-${PLATFORM}.exe" -DUNINSTALL_MANIFEST="${UNINSTALL_MANIFEST}" -DINSTALL_SIZE_KB="${INSTALL_SIZE_KB}" ${USE_PROGRAMFILES32} ${USE_DISCORDID} buildpackage.nsi
	popd > /dev/null
