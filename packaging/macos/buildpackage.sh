...
# 10.15 is the floor for every slice: it is the oldest macOS .NET 6 supports, and there is no
# longer any other runtime that could serve anything older.
...
install_mod_assemblies "${TEMPLATE_ROOT}" "${LAUNCHER_ASSEMBLY_DIR}/x86_64" "osx-x64"
install_mod_assemblies "${TEMPLATE_ROOT}" "${LAUNCHER_ASSEMBLY_DIR}/arm64" "osx-arm64"
