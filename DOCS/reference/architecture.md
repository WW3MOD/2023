...
### Terrain decals and actors are two sequential PASSES, so no `ZOffset` can put a smudge over an actor

The tempting lever points the other way: the mod already carries `WithSpriteBody.ZOffset: -8192` on `^CivField` with a long comment explaining why fields must render under units, which makes "make the scar draw over the field" look like a sorting problem with an obvious dual. It is not one.

`IRenderOverlay` — how every `SmudgeLayer`, `ResourceRenderer` and `BuildableTerrainLayer` draws — is called from `TerrainRenderer.RenderTerrain` (`TerrainRenderer.cs:112`), i.e. from inside `WorldRenderer.Draw`'s terrain step at `:378`. Actor sprites are the `preparedRenderables` loop at `:388-389`. **Those are two sequential passes, not one sorted list**, so a terrain decal is not "below" an actor in any depth sense a `ZOffset` could reorder — it is drawn earlier, full stop, and `ZOffset` only ever sorts renderables *within* the second pass. Consequence: **a smudge on a cell occupied by any actor whose sprite covers the cell is placed, is present in the layer, and is invisible.** On farmland that is most of the map.
