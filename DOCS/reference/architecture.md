...
**A validity readout must BE the production predicate, not a reimplementation of it.** A border that does not split the map is not a weak border, it is no border: `DefconWallRegion.IsDegenerate` is `ComponentCount < 2`, and `BuildRegion` then logs one line and leaves the wall down for the whole match. The editor's live *"DMZ splits the map into N areas"* readout therefore computes N by **constructing `DefconWallRegion` itself** (`ZoneLayerOverlay.ComponentCount`) with the same `Map.Contains` passability predicate `BuildRegion` passes. A lookalike flood differing in 4-vs-8 connectivity, `Bounds` vs `AllCells`, or the predicate would tell the mapper their border is fine and then not raise it — *a readout that can disagree with the runtime is worse than none, because it is trusted.*
...
**A plain owner change does NOT recolour the ghost sprite; it moves the tooltip and nothing else visible.** `FrozenUnderFog.UpdateFrozenActor` (`:97`) sets a `VisibilityHash` bit and calls `RefreshState`, and `RefreshState` (`FrozenActorLayer.cs:135-155`) writes `Owner`, `TargetTypes`, `targetablePositions`, `HP`, `DamageState`, `TooltipInfo` and `TooltipOwner` — **it touches no renderable.** `NeedRenderables` has exactly three writers engine-wide: the `FrozenActor` constructor from `startsRevealed` (`:93`), `UpdateVisibility` on a false→true visibility edge (`:207`), and `FrozenUnderFogUpdatedByGps.cs:43`; it is read and cleared at `FrozenUnderFog.cs:171`/`:184`. So the cached `Renderables` array keeps the old owner's colours until the viewer **re-observes**, and the observable leak on a capture is the tooltip (plus the GPS dot), not the sprite. Note also that `FrozenUnderFog.OnOwnerChanged` (`:221`) refreshes `frozenStates[oldOwnerIndex]` **only** (`:243-244`) — a third party's ghost is untouched by an ownership change, which is the opposite of what the symptom ("the ghost shows the wrong owner") suggests.
...
`GarrisonManagerInfo.Indestructible` also defaults `true` (`:83-85`), again with **zero YAML overrides**, and it applies to **all 41 garrisonable actors** *(re-derived 2026-09-02 from the inheritance closure of `^CivBuilding` plus `GTWR`/`PBOX`/`HBOX` — the only four `GarrisonManager:` declarations in the mod, and there is no `-GarrisonManager` anywhere)*. The damage modifier returns 0 once `health.HP <= 1` (`:1417-1435`, test at `:1419`) **with no occupancy gate** — an empty garrisonable building is exactly as unkillable as a full one.

Every trait that fires on death is therefore unreachable by ordinary damage on those actors, including the `SpawnActorOnDeath` husks (`PBOX` → `pbox.husk` at `structures-defenses.yaml:216-217`, `HBOX` → `hbox.husk` at `:311-312`; `GTWR` has none) and `V19` → `V19.Husk` (`civilian.yaml:431-432`). **The one channel that still reaches them is `Actor.Kill()`**, which calls `InflictDamage(…, ignoreModifiers: true)` (`Health.cs:245`) and so skips the modifier chain (`:177`). If one of these buildings has to die in a test or a script, that is the only door.
...
**The clamp is the part with a trap in it, and it is not in this file.** `GarrisonManager.Indestructible` holds the building at 1 HP by returning a damage modifier of **0** (`GarrisonManager.cs:1451-1468`), and `Health` applies modifiers *before* raising `INotifyDamage` (`Health.cs:177-215`) — so a naive `Damaged` reads zero damage at the clamp and forwards nothing, making a rubbled garrison **immune**, which is the opposite of what the curve says. `GarrisonProtection` therefore also implements `IDamageModifier` purely as an observer (always returns 100) to record what the attacker *aimed* before the clamp erases it, and `EffectiveIncomingDamage` picks between the two. `GarrisonRubbleProtectionTest` pins the whole curve monotone from full health to the clamp for every `GarrisonProtection` block in the mod.
