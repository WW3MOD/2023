...
| [`architecture.md`](architecture.md) | Engine layout, scenario system, custom traits, aircraft movement, suppression/stances, AI config, determinism & sync, the LOS shadow cache, asset & audio/music pipeline, the render pass order & fog compositing, the kill hook, networking/NAT, saved games, replays & build fingerprint, widget gotchas |
...
**Code-side twin.** These four shapes are about *claims in prose*. The same failure in *shipped code* — a mechanism that is present, documented, wired up, lint-clean, and evaluates to nothing — is catalogued with six worked instances in [`conventions.md` §"A change believed made, documented as made, and inert"](conventions.md#a-change-believed-made-documented-as-made-and-inert). Read them together: both reduce to *name the line you would delete to break the property, and check that deleting it would change anything.*

**A fifth shape, about EVIDENCE rather than about code: a row asserting what a commit DID, filled from its message.** *(Promoted 2026-09-20 from a 2026-08-22 entry, whose own history table contained two instances of it.)* A commit message that announces a fix necessarily **describes the broken behaviour in the present tense first**, and a table column headed "what it did" invites that description to be copied into the row. Two of five rows in one such table inverted this way: one commit was blamed for damage its message was quoting from its *predecessor*, another for a behaviour its message was quoting as the *fix*. Both read as careful work, and both survived because the source looked authoritative. *Detector: **a row asserting what a commit DID must be filled from its diff.** A message may be quoted as evidence of intent and never as evidence of effect.* The audit that found the second instance re-read all nine commits the section cited; seven were clean, so this is a real failure rate on a real corpus, not a hypothetical.
