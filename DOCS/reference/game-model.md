...
**Which edge is not reliably the Supply Route's — for REINFORCEMENT ENTRY this is a statement about the `spawnarea` hint only, and for EVACUATION it is no longer true at all.** *(Corrected 2026-09-02 away from an unconditional "nearest the Supply Route's SpawnArea hint", which was wrong on 9 of the 10 shipped maps. **Corrected again 2026-09-21 at `main @ 70e63582`:** the 09-02 text concluded that "both reinforcement entry and evacuation resolve from wherever the actor happens to be standing, **not** from the SR", and that sentence is now wrong on both halves, for two different reasons. It is quoted here rather than deleted because work downstream of it is still in the tree.)* **Two different traits are involved and they must not be read as one.** (1) **Reinforcement entry** is `ProductionFromMapEdge`, whose own `FindClosestSpawnArea` (`ProductionFromMapEdge.cs:57`) falls back to `?? self.Location` (`:101`, `:119`) — and `self` there is the **producing building**, i.e. `SUPPLYROUTE` (the trait is declared at `structures.yaml:472`, inside the `SUPPLYROUTE:` actor opening at `:295`). So entry without a `spawnarea` resolves from the SR's **own cell**, which is the SR. What the `spawnarea` hint changes is *which cells on that edge* are offered, not whether the anchor is owner-side. (2) **Evacuation** is `RotateToEdge`, and since item 78 (`ac16f2b6`, 2026-09-05) its ground branch reads `FindClosestSpawnAreaForOwner(self) ?? FriendlyEvacuationOrigin(self)` (`Activities/RotateToEdge.cs:209`) — the nearest **friendly `SUPPLYROUTE`** (own or allied), then `HomeLocation`, then the unit (`:158-170`). The old `?? self.Location` unit-anchored fallback is gone; measured before the change, it sent **70.4%** of raid-range evacuations out through an **opponent's** wall. The **aircraft** branch is untouched and uses `?? self.Owner.HomeLocation` (`:195`). Only `river-zeta-ww3` ships `spawnarea` actors (6, one per `mpspawn`); the other nine maps have zero, as do most autotest scenarios *(re-counted 2026-09-21)* — **that part of the 09-02 correction stands**, and it is still worth running `grep -c spawnarea` before asserting anything about the hint. What no longer follows from it is that the resolved anchor is unit-side: on every shipped map, both paths now anchor on the Supply Route. The `spawnarea` anchor mechanism itself — and the separate `AmmoPool` rearm-vs-evacuate comparison that still keys on `FindClosestSpawnAreaForOwner`, which item 78 deliberately did not touch — is documented in [`economy.md` §"The evacuation anchor is the `spawnarea` actor"](economy.md).
