...
**`AmmoPool.CannotFight` is the right "can this unit fight" predicate — and it exempts aircraft.** It is `AllPoolsEmpty(self) && !self.Info.HasTraitInfo<AircraftInfo>()` (`AmmoPool.cs:216`), so an airframe never reads as unable to fight through it (consistent with aircraft having no reachable rearm host — see "What rearms what"). Of the plausible alternatives, three are wrong: not `Rearmable.RearmableAmmoPools` (a different question), not the `^AmmoDecoration` red-pip condition (implied by, not equal to), and not "every armament `IsTraitPaused`" — that one is a *per-armament* question and the engine's own guidance at `AttackBase.cs:440-447` notes `CannotFight` is itself insufficient for the mixed case of a unit dry on its rifle but holding a loaded RPG.
