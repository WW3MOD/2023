...
- When an SR's control bar empties, a defeat bar fills; at full it calls `OnDefeatBarFull` (`SupplyRouteContestation.cs:354`) → the owning player goes **passive** (production frozen). Elimination fires only if that team has **no other active SR** (`ResolveTeamElimination`, `:423`).
