...
- When an SR's control bar empties, a defeat bar fills; at full it calls `OnDefeatBarFull` (`SupplyRouteContestation.cs:409`) → the owning player goes **passive** (production frozen). Elimination fires only if that team has **no other active SR** (`ResolveTeamElimination`, `:478`, reached via `HasActiveTeamSupplyRoute`, `:433`).
  ...
  - **The system line overstates: income is NOT frozen.** `OnDefeatBarFull` prints `"… has lost their Supply Route! Production and income frozen."` (`:417`), but the trait's only behavioural hook is `IProductionSpeedModifier` (interface list at `:101-102`) and it contains no reference to `PlayerResources`, cash or income anywhere. A passive player keeps accruing the passive-income stream (see [`economy.md`](economy.md) §Where cash comes from). Production genuinely does halt; the second half of that sentence is wrong. *(Flagged 2026-08-19 during curation — the message is C#, so it is left for a code change rather than edited here. Note also that the public `IsPassive` accessor at `:186` has **zero call sites** in the repo, so nothing else consumes passivity either.)*
