...
- **The SR is indestructible by design — but NOT because of its armor.** It cannot be killed by damage, and the mechanism matters because the obvious-looking one is inert. `Armor: Type: Indestructable` (`structures.yaml:317-318`) does **nothing**: an armor type only bites through a warhead's `Versus:` table, `Indestructable` appears nowhere else in the mod, and `DamageWarhead.DamageVersus` returns an unmodified 100% for an armor type no warhead lists (`DamageWarhead.cs:96-108`; `ArmorInfo.Type` has no other effect — `Armor.cs:20-21`). What actually protects the SR is `Targetable: TargetTypes: NoAutoTarget` (`structures.yaml:296-297`): that is its *only* target type, warheads default to `ValidTargets: Ground, Water` and no weapon in the mod lists `NoAutoTarget`, so `Warhead.IsValidAgainst` rejects every warhead before damage is computed (`Warhead.cs:55-74`). **Consequence: adding an ordinary target type (`Ground`, `Structure`) to that one line turns the SR into a destructible 75000-HP building and silently changes the whole defeat model — note the line must be REPLACED, not deleted, since an absent `TargetTypes` leaves an empty `BitSet` (`Targetable.cs:23` has no initializer) that is equally untargetable — while the `Armor: Indestructable` line sits there looking like a guarantee it does not provide.** **Contestation** is the only *implemented* pressure mechanic (graduated production slowdown via `SupplyRouteContestation` while enemies stand inside the 10-cell contestation circle). Capture is part of the intended design but **is not wired in the code today** — see the Capture section below for what actually happens.
...
The match-ending win/loss runs on **`SupplyRouteContestation`, not stock `ConquestVictoryConditions` alone.** The SR is never destroyed (no warhead can target it — see §"indestructible by design" above for why `Armor: Indestructable` is *not* the reason), so "destroy the conyard = defeat" is replaced by "contest the SR to zero":
