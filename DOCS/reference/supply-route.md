...
- When an SR's control bar empties, a defeat bar fills; at full it calls `OnDefeatBarFull` (`SupplyRouteContestation.cs:409`) → the owning player goes **passive** (production frozen). Elimination fires only if that team has **no other active SR** (`ResolveTeamElimination`, `:478`, reached via `HasActiveTeamSupplyRoute`, `:433`).
  - **In a 1v1 that condition is always false, so passive and Lost land in the SAME TICK — "passive" is not a survivable state outside team games.** `HasActiveTeamSupplyRoute` scans for *another* actor on the same team (`:435-445`); with no teammates it returns false unconditionally, so `ResolveTeamElimination` always follows. Reading only as far as the comment above the call gives the opposite impression, that the mechanic is purely economic and reversible. **Reversibility is real but sits earlier**: the defeat bar drains and `OnReinstated()` fires if the contesting force is cleared *before* the bar fills (`:325-345`).
  - **The freeze notifications fire only when a rescue is actually possible, and they no longer claim income stops.** Both halves were wrong until 2026-08-22. (a) The system line was unconditional, so a free-for-all player read `"… has lost their Supply Route!"` on the line immediately above `"… is defeated"` — announcing a recoverable state they occupied for exactly one tick. `OnDefeatBarFull` (`:551`) now evaluates `HasActiveTeamSupplyRoute()` **once** into a local and forks on it: announce-and-freeze when true, `ResolveTeamElimination` when false. Gating on the *same* expression that decides passive-versus-defeated is the point — never on a re-derived "is this a team game?", because a lobby team of one, the last survivor of a team, a player whose only ally is itself passive, and an FFA player are indistinguishable here and all unrescuable. The decision is extracted as the pure `HasRescuer` (`:592`) and NUnit-pinned in `SupplyRouteEliminationTest.cs`. (b) The wording said "Production **and income** frozen" in both the system line and `PassiveTextNotification`; income is not frozen — the trait's only behavioural hook is `IProductionSpeedModifier` and nothing reading `IsPassive` touches `PlayerResources`, so a passive player keeps accruing the passive-income stream (see [`economy.md`](economy.md) §Where cash comes from). Both now read "Production frozen." *(The public `IsPassive` accessor at `:221` still has **zero call sites** repo-wide, so nothing outside this trait consumes passivity at all.)*
