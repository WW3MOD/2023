...
- **⚠️ RIGHT-CLICKING YOUR OWN SUPPLY ROUTE SELLS THE SELECTED UNIT, IRREVERSIBLY, UNDER A GENERIC `enter` CURSOR — and the SR cannot be right-clicked to move.** *(Recorded here 2026-09-01; this is the only player-facing consequence of the SR that is not otherwise reachable from this doc.)* The SR carries `AcceptsDeliveredCash` with `ValidTypes: Rotation` (`structures.yaml:370-371`), and **every combat unit carries `DeliversCash@Rotation`** (`vehicles.yaml:102`, `aircraft.yaml:148, 198`, `infantry.yaml:156`). Its targeter is `OrderPriority` **5** with `Cursor = "enter"` (`DeliversCash.cs:150`, `:39`), which **outranks `Mobile.MoveOrderTargeter`'s 4** (`Mobile.cs:1220`) — hence both halves of the symptom.
