...
- **The SR is UNTARGETABLE by design, not indestructible — but NOT because of its armor.** It cannot be killed by damage, and the mechanism matters because the obvious-looking one is inert. `Armor: Type: Indestructable` (`structures.yaml:368-369`) does **nothing**: an armor type only bites through a warhead's `Versus:` table, `Indestructable` appears nowhere else in the mod, and `DamageWarhead.DamageVersus` returns an unmodified 100% for an armor type no warhead lists (`DamageWarhead.cs:96-108`; `ArmorInfo.Type` has no other effect — `Armor.cs:20-21`). What actually protects the SR is `Targetable: TargetTypes: NoAutoTarget` (`structures.yaml:347-348`): that is its *only* target type, warheads default to `ValidTargets: Ground, Water` and no weapon in the mod lists `NoAutoTarget`, so `Warhead.IsValidAgainst` rejects every warhead before damage is computed (`Warhead.cs:55-74`). **Consequence: adding an ordinary target type (`Ground`, `Structure`) to that one line turns the SR into a destructible 75000-HP building and silently changes the whole defeat model — note the line must be REPLACED, not deleted, since an absent `TargetTypes` leaves an empty `BitSet` (`Targetable.cs:23` has no initializer) that is equally untargetable — while the `Armor: Indestructable` line sits there looking like a guarantee it does not provide.** **A SECOND PROTECTION EXISTS SINCE 2026-09-08 AND IS EQUALLY LOAD-BEARING: `-Vaporizable:` at the top of the `SUPPLYROUTE:` actor.** `VaporizeWarhead` deliberately does not consult target types (`VaporizeWarhead.IsValidAgainst`), because the actors it exists to remove — the neutral tech buildings — are protected by the same `NoAutoTarget` idiom. Every nuke in the arsenal now carries one. Deleting the `-Vaporizable:` line makes an SR die to the cheapest warhead in the game, and by `MustBeDestroyed: RequiredForShortGame` that is **immediate defeat**, not a lockout. `VaporizeScopeTest.TheSupplyRouteOptsOutOfVaporisation` goes red on that deletion. **Contestation** is the only *implemented* pressure mechanic (graduated production slowdown via `SupplyRouteContestation` while enemies stand inside the 10-cell contestation circle). Capture is part of the intended design but **is not wired in the code today** — see the Capture section below for what actually happens.
...
**The SR is at `spawn + (-1,-1)`, never at the spawn cell — so SR parity is the INVERSE of spawn parity.** `SpawnStartingUnits` places the base actor at `p.HomeLocation + unitGroup.BaseActorOffset` (`SpawnStartingUnits.cs:91`), and `BaseActorOffset` defaults to `CVec(-1, -1)` (`MapStartingUnits.cs:37`). Nothing overrides it — zero occurrences anywhere under `mods/`, and no map places a `supplyroute` actor explicitly. Two consequences that have each cost real time:

- **Any parity argument read off `map.yaml` spawn coordinates is inverted at the SR.** This matters because the influence stack's grid is aliased on cell parity (see [`influence-stack.md`](influence-stack.md) §Stage B): a guard that is healthy only on odd/odd SRs needs an **even/even spawn**. Derive the SR cell before reasoning about it; do not reason from the spawn cell. *(Worked trap, spawns as of `de78a1ed`: `siberian-pass-ww3` is the only two-player map whose spawns are both odd/odd — `95,15` and `1,51` — which makes it look like the ideal clean measurement map, and its SRs are `94,14` and `0,50`, both even/even and therefore both polluted. Among the maps a match is actually played on, `twin-rivers-ww3` slots `112,92` and `112,28` are the only pair that both land on odd/odd SRs. The two special-purpose maps `arena-tank-duel` and `shellmap-open-field` also have all-even spawns, so they qualify too — the source entry called twin-rivers "the only map", which is true only if you exclude those.)*
- **A scenario with an `mpspawn` has one more SR than its `map.yaml` lists** — an own-player `supplyroute` at `spawn + (-1,-1)`, alongside any the map places. It is harmless (an own SR is not an offensive target, and `SUPPLYROUTE` carries no `CaptureManager` so nothing dispatches a capturer at it) but it is invisible in the actor list and shows up only as a POI. **A scenario that counts SRs, or assumes its own SR is unique, counts wrong.**
...
- `SUPPLYROUTE` (`structures.yaml:222`) has **no `Capturable` and no `CaptureManager`**, and inherits none — its bases are `^ExistsInWorld` / `^SpriteActor` / `^SelectableBuilding`, none of which pull in the capture chain (`^NeutralOrOccupiedCapturable`, `structures.yaml:169`). There is no capture-type for a TECN's `Captures{building-neutral/-occupied}` to intersect, so a technician has nothing to enter. The `CaptureNotification` at `structures.yaml:236` is commented out and wires nothing.
- **`OwnerLostAction: ChangeOwner → Neutral` (`structures.yaml:270-272`) does NOT fire on capture.** `OwnerLostAction` implements `INotifyOwnerLost` — "when the actor's owner is **defeated**" — and `OnOwnerLost` is called *only* from `ConquestVictoryConditions.cs:109` and `StrategicVictoryConditions.cs:152`, both iterating a just-defeated player's actors. So an SR goes Neutral **only when its owning player loses the match**, never via an engineer.

One subtlety for whoever implements this to match the design: neutral-SR capture needs `Types: building-neutral`, enemy-SR needs `building-occupied`.

The second subtlety this section used to record — that "the capturer can never keep it, it just neutralizes" needs a custom on-capture hook because stock `Captures`/`Capturable` always transfers to the capturer — **no longer applies. That hook now exists**: `CaptureToNeutral` on `CapturesInfo` (`Captures.cs`) transfers the target to the world owner instead of the captor, and `EnterBehaviour: Exit` returns the capturing unit alive. Both are honoured in `CaptureActor.DoCapture` and both default off. They were built for the soldier-clears-buildings rule (see [`../gameplay/capturing.md`](../gameplay/capturing.md)) and are directly reusable here — SR capture would be `CaptureToNeutral: true` on whichever template gets the SR capture type.
...
The match-ending win/loss runs on **`SupplyRouteContestation`, not stock `ConquestVictoryConditions` alone.** The SR is never destroyed (no warhead can target it, and the one warhead type that ignores target types is opted out at the actor — see §"UNTARGETABLE by design" above for why `Armor: Indestructable` is *not* the reason), so "destroy the conyard = defeat" is replaced by "contest the SR to zero":

*(Line citations in this subsection re-derived and corrected 2026-09-20 at `a21583fd`; the previous set predated a ~300-line growth of `SupplyRouteContestation.cs` and pointed into unrelated code. The mechanism they describe is unchanged and was re-read in full.)*

- When an SR's control bar empties, a defeat bar fills; at full it calls `OnDefeatBarFull` (`SupplyRouteContestation.cs:702`, called from `:572`) → the owning player goes **passive** (production frozen). Elimination fires only if that team has **no other active SR** (`ResolveTeamElimination`, `:810`, reached via `HasActiveTeamSupplyRoute`, `:745`).
  - **In a 1v1 that condition is always false, so passive and Lost land in the SAME TICK — "passive" is not a survivable state outside team games.** `HasActiveTeamSupplyRoute` scans for *another* actor on the same team (`:745-748`, over `OtherSupplyRoutes` `:752`); with no teammates it returns false unconditionally, so `ResolveTeamElimination` always follows. Reading only as far as the comment above the call gives the opposite impression, that the mechanic is purely economic and reversible. **Reversibility is real but sits earlier**: the defeat bar drains and `OnReinstated()` fires if the contesting force is cleared *before* the bar fills (`:992`, called from `:602`).
  - **The freeze notifications fire only when a rescue is actually possible, and they no longer claim income stops.** Both halves were wrong until 2026-08-22. (a) The system line was unconditional, so a free-for-all player read `"… has lost their Supply Route!"` on the line immediately above `"… is defeated"` — announcing a recoverable state they occupied for exactly one tick. `OnDefeatBarFull` (`:702`) now evaluates `HasActiveTeamSupplyRoute()` **once** into a local and forks on it: announce-and-freeze when true, `ResolveTeamElimination` when false. Gating on the *same* expression that decides passive-versus-defeated is the point — never on a re-derived "is this a team game?", because a lobby team of one, the last survivor of a team, a player whose only ally is itself passive, and an FFA player are indistinguishable here and all unrescuable. The decision is extracted as the pure `HasRescuer` (`:771`) and NUnit-pinned in `SupplyRouteEliminationTest.cs`. (b) The wording said "Production **and income** frozen" in both the system line and `PassiveTextNotification`; income is not frozen — the trait's only behavioural hook is `IProductionSpeedModifier` and nothing reading `IsPassive` touches `PlayerResources`, so a passive player keeps accruing the passive-income stream (see [`economy.md`](economy.md) §Where cash comes from). Both now read "Production frozen." *(The public `IsPassive` accessor at `:262` still has **zero call sites** repo-wide, so nothing outside this trait consumes passivity at all.)*
- **The win is awarded explicitly, per survivor, on ANY defeat path.** Older code only marked the *losing* team and relied on stock `ConquestVictoryConditions.Tick` to infer the win — which fails in a near-simultaneous mutual overrun (both resolve as `Lost` before the inference tick, so everyone shows "mission failed"). The award is now a path-independent `SupplyRouteContestation.AwardDecidedSurvivors(World)` (`SupplyRouteContestation.cs:475`) called synchronously from `ConquestVictoryConditions.OnPlayerLost` (`:119`) — so it fires the instant *any* player is defeated (SR contested to zero, loss of all required units via `MarkFailed`, surrender, or a mutual same-tick overrun), not only from the SR-contested `OnDefeatBarFull` path where the original award lived. It marks `Won` each survivor whose every non-allied combatant is now `Lost` (`ShouldAwardVictory`), reproducing CVC's per-survivor test so FFA / 2v2v2 don't instantly resolve when one party dies. Idempotent — it only touches `Undefined`/`Incomplete` objectives, so a second defeat in the same tick is a no-op (the anti-"everyone loses" invariant).
...
- **Awarding via `MissionObjectives`:** `MarkCompleted` fires `OnPlayerWon` only when **all** required objectives are Completed (`MissionObjectives.cs:136-137`). `AwardVictory` (`:580`) therefore no-ops unless the player has a `ConquestVictoryConditions` trait and completes only its `Type == "Primary"` objectives — never blanket-completing (which would auto-win a scripted campaign mission running `-ConquestVictoryConditions` + `MissionObjectives.EarlyGameOver`).
- **`TestMode` symmetry:** `ResolveTeamElimination` early-returns on `TestMode.IsActive` (`:814`), matching `ConquestVictoryConditions.Tick` (`:63`) / `MissionObjectives.CheckIfGameIsOver` (`:171`), so an SR contest emits no stray victory lines mid-autotest. Consequence for test authors: the whole interactive win/loss *verdict* is suppressed under `TestMode`, so a regression test for win attribution cannot observe a game-over — it must read `Player.WinState` directly (Lua getter, `PlayerProperties.cs:72`). `AwardDecidedSurvivors` itself is NOT `TestMode`-gated (it fires from `OnPlayerLost` regardless), so the underlying Won/Lost state is set and observable even while the end-screen stays suppressed. The pure branches (`SelectEliminationTargets` / `ResolveEliminationOutcome` / `ShouldAwardVictory` / `TeamHasVictor`) are NUnit-pinned (`SupplyRouteEliminationTest.cs`); the full team-propagation ending is verified by unit test + reasoning (no single autotest map runs 2v2 team victory).
...
- **Actor definition:** `mods/ww3mod/rules/ingame/structures.yaml:222` — `SUPPLYROUTE` block. The physical building is a **3×3 footprint** (`=+= +++ =+=`, `Dimensions: 3,3`, `structures.yaml:285-286`), which matters for placement math: a top-left-anchored 3×3 near a map corner can overflow the map bounds, so corner SRs sit a few cells inward. Note: **no `Capturable`/`CaptureManager`** here or in any inherited template — capture is unimplemented (see the Capture section).
- **Neutralize-on-defeat:** `OwnerLostAction` (`structures.yaml:270-272`) → fires only from `ConquestVictoryConditions.cs:109` / `StrategicVictoryConditions.cs:152` when the owner is defeated — **not** a capture path.
- **Spawn wiring:** `mods/ww3mod/rules/world.yaml:316–388` — every `StartingUnits@*` has `BaseActor: supplyroute`.
- **Contestation trait:** `SupplyRouteContestation` (engine side, paired with `WithRangeCircle@Contestation` for the visual), attached at `mods/ww3mod/rules/ingame/structures.yaml:303` inside the `SUPPLYROUTE:` actor that opens at `:222`. *(Both the path and the line were stale in `CLAUDE.md` — it cited `structures.yaml:260` without the `ingame/` segment — corrected 2026-08-30. The trait itself and every timing around it are unchanged.)*
  - **It is an ACTOR trait on the structure, not a player trait, which is the thing to know before building a per-player readout on it.** `Info`, `ControlBar`, `ControlBarFraction`, `NetEnemySurplus`, `NetFriendlySurplus`, `TeamValue` and `IsPassive` are all public (`SupplyRouteContestation.cs:220-226`), but reaching them for a given player means `world.ActorsHavingTrait<SupplyRouteContestation>()` filtered by owner — there is no `PlayerActor.Trait<SupplyRouteContestation>()` to ask.
...
- **⚠️ RIGHT-CLICKING YOUR OWN SUPPLY ROUTE SELLS THE SELECTED UNIT, IRREVERSIBLY, UNDER A GENERIC `enter` CURSOR — and the SR cannot be right-clicked to move.** *(Recorded here 2026-09-01; this is the only player-facing consequence of the SR that is not otherwise reachable from this doc.)* The SR carries `AcceptsDeliveredCash` with `ValidTypes: Rotation` (`structures.yaml:370-371`), and **every combat unit carries `DeliversCash@Rotation`** (`vehicles.yaml:133`, `aircraft.yaml:170, 220`, `infantry.yaml:156`). Its targeter is `OrderPriority` **5** with `Cursor = "enter"` (`DeliversCash.cs:150`, `:38`), which **outranks `Mobile.MoveOrderTargeter`'s 4** (`Mobile.cs:1258`) — hence both halves of the symptom.
  - **This is SETTLED and deliberate — do not re-file it as a bug.** The full reasoning, including why the `Rotation` branch discards the clicked target and walks the unit to the map edge instead, is written in place at `DeliversCash.cs:101-114`. **Read it there rather than re-deriving it** *(one home per fact)*. One imprecision in that comment: it cites `structures.yaml:222` for `AcceptsDeliveredCash`, which is where the `SUPPLYROUTE:` actor opens — the trait itself is at `:370-371`.
