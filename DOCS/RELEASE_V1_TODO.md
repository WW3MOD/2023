...
- [ ] Medics, for example, have a problem when ordering a group of soldiers to attack move, the medics cant fire so they jsut keep running towards the enemy and end up getting killed and not being useful.
    ...
    - **NEEDS DESIGN DISCUSSION** — Skipping for now. My initial thoughts (pick or override):
      - **Option A (cheap, helps a lot):** Add `Selectable: Priority: 5` on medics so drag-select prefers riflemen. Combined with the existing 1-medic-per-soldier claim system, medics that *do* get selected stay back in defensive stance.
      - **Option B (the one you described):** New 5th stance bar, "Follow stance" — `FollowGroup` (default for medics) / `Independent` (default for everyone else). FollowGroup units pick the nearest allied combat unit and trail it at ~3-5 cells; if a heal target appears, they break off, heal, then resume following. Implement as a new trait `AutoFollowAlly` listening for OnIdle.
      - **Option C (simpler, no new bar):** Tie auto-follow to the existing engagement stance — `HoldPosition` = stay put (no follow), `Defensive` = follow at short range, `Hunt` = roam freely. That's nice because it reuses existing UI but it overloads "engagement" with movement intent.
      - I lean B if you want a clean toggle, C if we want to keep the UI minimal. Which do you prefer?
...
- [ ] When units move over tiles with a lower move speed/mobility according to their "locomotor", they become slow from what seems like the point when they are in the center of the "fast" tile, all the way to when they reach the cetner of the "slow" tile and then they speed up again. So the problem with this is that they start slowing down before they even enter that tile, and sped up before they have left it. We should slow them down the moment they enter the slow tile, and speed up when they exit it.
    - DEFERRED — engine work, needs careful change to MovePart. The current code reads the locomotor speed at FromCell or ToCell at boundaries; need to use ToCell for the entire transition (so speed reflects the destination tile, not a midpoint average). Will tackle next session.
...
- [ ] We should be able to target the Supply Route, and it should be the default order for any unit with a weapon, and that order means Go to the flag and stay there until it is fully captured/ceutralized, so that we can order an "attack" on the SR and then queue up other orders after it is captured.
    - **NEEDS DESIGN DISCUSSION** — Skipping for now. My understanding:
      - Right-click on enemy SR with a combat unit → unit walks to SR's contestation radius and STAYS there.
      - When SR is fully neutralized (bar drops to 0), the order completes and the unit moves on to the next queued order.
      - Implementation: new order `AttackSupplyRoute` that's an attack-move-style activity but treats "neutralize" as the completion condition. Need to confirm the SR's `SupplyRouteContestation` trait already exposes "is neutralized" state — if so, this is mostly a new activity + cursor wiring.
      - Question: should this also work for friendly SRs (defending) — i.e., move there and stay until enemy threat is gone? Or only enemy SRs?
...
- [ ] Medics on "defensive" stance should still "hunt" for hurt soldiers to heal, but at a reduced range, maybe 5 tiles. The problem arises from the limitation that hunt is very long range, and medics might end up running across the battlefield and getting killed. We want to be able to differentiate between long (hunt), short (defensive) and hold position completely. This is how we have designed the new settings for all units basically, but the defensive stance doesnt seem to make any units move at all. For regular soldiers defensive stance means they can reposition if they need to in order to aquire a targeting solution (LoS), but they should not follow the enemy, instead they jsut moves perhaps a little bit to not be blocked, fire and then return to their original position. Hunt means go after enemies, do no return to the original position.
    - DEFERRED — Two distinct changes needed:
      1. **Defensive stance: short-range hunt for healers** — pass a `DefensiveRange` (~5c0) to `HealerAutoTarget` and gate `FindBestTarget` by it when on Defensive stance.
      2. **Defensive stance for combat units: reposition + return** — the current Defensive stance does nothing for movement. Need to:
         - Cache the original position when entering Defensive stance.
         - Allow short-range moves (1-3 cells) to acquire LoS.
         - After firing, return to origin if no further targets in range.
       - This is a chunky bit of engine work and would benefit from a separate design conversation.
