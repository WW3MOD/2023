...
- **Behavioral fixes ship with their auto-asserting test** (RED → GREEN proof). The test scenario lives in `tools/autotest/scenarios/test-*/` and gets committed alongside the fix. See `DOCS/recipes/AUTOTEST.md` for the loop and the "default-to-AUTOTEST" checklist.
