...
| **Claims** | `BuildFreePool()` (`:1932-1942`) = `IsEligibleCombatUnit` minus axis-claimed minus ledger-committed. The predicate (`:2317-2420`): owned/alive/in-world, has `IPositionable` **and** `AttackBase`, **not** `Aircraft`, not out-of-ammo (under `SkipOutOfAmmoUnits`), not STARVING (under `StarvingRecruitThresholdPerMille` — `SkipOutOfAmmoUnits` is an *empty* test and does nothing at 10%; see `StarvingRecruitGate`, shared with five other recruiting modules), not evacuating, **not** `CrewMember`, and under `UseUnitRoles` role ∈ {`MainBattle`, `IndirectFire`} and **not** a troop carrier. Notably it does **not** filter on `IsIdle`. |
