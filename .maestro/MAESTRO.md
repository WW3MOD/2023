...
- **Shared-HEAD hazard — the stronger form of the same thing, and the index rule above is what hid it (2026-09-01).** A checkout shares its HEAD as well as its index. A doc worker told to work "in the main checkout on a branch" runs `git checkout -b wt/whatever` and **moves HEAD for every party in that directory, including the manager**. Observed live: the manager's in-progress edit to `mods/ww3mod/lint-baseline.txt` was sitting on the worker's branch, and a `git checkout main` by either party at the wrong moment would have yanked the tree out from under the other mid-edit. Caught before anything was committed, purely by chance — the manager happened to run `git rev-parse --abbrev-ref HEAD` for an unrelated reason.
  **Rule: never write a brief that has a worker run `git checkout` in the shared checkout at all.** Doc-only work is not the exception it looks like — it is cheap to give a doc worker its own worktree (`git worktree add <path> -b <branch> main`, no build needed for docs), and the only thing the shared checkout buys is skipping that one command.
  The generalisable failure: the existing bullet named the index, so "commit path-limited" read as the complete countermeasure and the brief looked compliant while being unsafe. **A hazard note that names one mechanism will be read as naming the only one** — when recording a shared-resource trap, say which resources are shared, not just which command misbehaves.
