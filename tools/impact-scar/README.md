...
| `scar_density.py` | Measures an autotest SCREENSHOT rather than rendering one: per named cell, the fraction of its ground pixels that are bright unburnt wheat, compared only against neighbours in the same annulus bucket. Carries a geometry self-check, because the captures are 60 px per cell and not 48. |
