...
--   * e3.america  — TWO pools (5.56mm DMR + RPG), the exact tooltip the 2026-08-30 report was
--                   written against ("the tall rifleman tooltip lands across the sidebar's icon
--                   column"). This is the repro frame.
--   * sn.america  — one pool. The control: if the interior is opaque here but not on the two
--                   above, the bug is in the tiling, not the tile.
--
-- THE NAMES ARE FACTION-SUFFIXED, AND THAT IS NOT COSMETIC. The first version of this scenario
-- asked for `e3` and `e1` and both silently failed to arm (run 260922_011327): WW3MOD gates
-- infantry behind `~player.<faction>`, so the buildable rifleman is `e3.america` while the bare
-- `E3` carries `Prerequisites: ~disabled` (infantry.yaml) and is in no queue at all. `e1.america`
-- is worse — the CONSCRIPT is `~disabled` for america too (infantry-america.yaml:5, with the real
-- prerequisites commented out beside it), so no spelling of it would have worked and the sniper
-- replaces it. `abrams` needs no suffix, which is why frame 01 worked and the other two did not.
--
...
local subjects = {
	...
	{ type = "e3.america", label = "02-rifleman-two-pools",
	...
	{ type = "sn.america", label = "03-sniper-single-pool",
	  note = "expects: the SINGLE-POOL case — one weapon section, 7.62MM SNIPER, AMMO 50 rounds, "
	      .. "REFILL, and a FULL REFILL total that is present even though this unit has only ONE "
	      .. "pool (that row was gated on having two until 0d7663ab). Interior solid as above. "
	      .. "NO-RESULT if the FULL REFILL row is absent, or if this frame shows the same tooltip "
	      .. "as 01 or 02 — that means the hover did not switch subject, and lua.log will say "
	      .. "armed=false." },
}
...
WorldLoaded = function()
	...
	for i, s in ipairs(subjects) do
		-- CLEAR FIRST, from the second subject on. If the hover below fails to arm, this frame
		-- must photograph an EMPTY panel, not the previous subject's tooltip — the failure mode
		-- that made run 260922_011327's frames 2 and 3 look like plausible wrong answers instead
		-- of visible breakage.
		if i > 1 then
			t = t + HoverSettleTicks
			Trigger.AfterDelay(t, function() Test.HoverProductionIcon("") end)
		end
