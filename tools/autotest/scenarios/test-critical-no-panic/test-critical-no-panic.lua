...
local ObserveTicks = 300      -- 18s: 300 ticks at the mod's real 16.667 tps (mod.yaml Timestep 60)
...
-- 325 polls, while AssertWithin(20) was 500 ticks at the harness's old hardcoded 25 and is 333 now
-- that the harness converts at the engine's real 16.667. Eight ticks short: the predicate could
-- never reach `ticks >= ObserveTicks`, and the run would fail on "observation window never
-- completed" having measured nothing at all.
--
-- Dividing by the same constant the helper converts with round-trips exactly (checked at the
-- shipped 16.667 and at 25), so this is 375 ticks whatever the constant says. Do not turn it back
-- into a literal.
