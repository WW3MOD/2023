...
-- reachability logs accumulate, then Test.Pass() so the runner exits and the
...
WorldLoaded = function()
	...
	-- forms and pushes. Then pass so the runner writes a verdict and exits.
	Trigger.AfterDelay(100 * TestHarness.TicksPerSecond, function()
		Test.Pass("frontline-reachability observation window elapsed — read debug.log [exp-reach] / [exp-offense] axis lines")
	end)
end
