...
2. **Three of the user's premises are wrong or shaky** and are corrected in §3: (a) ~~prone does **not** reduce detection in this engine, so "go prone to stay hidden" is cosmetic~~ **[corrected 2026-07-29 — this original bullet is WRONG; prone DOES grant +1 concealment tier via `DetectableAddativeModifier@Prone`, `infantry.yaml:684-686`; see the reclassified §3.1 and `DOCS/reference/architecture.md:262`. "Go prone to stay hidden" is a real, modest stealth lever, not cosmetic]**; (b) per-unit strength scans are **not** meaningfully costly at ambush cadence — the fear that drove "use map-layers instead" is largely unfounded; (c) "let the tank pass, shoot it in the back" is actively **punished** by the suppression model for the exact unit the user pictured (AT Specialist), and is doctrinally the *late* spring, not the optimal one.
...
**3.1 [HOLDS, corrected] "Go prone / stop to stay hidden."** Both halves hold, for different-strength reasons. **Prone does grant concealment: +1 detection tier.** `DetectableAddativeModifier@Prone` (`RequiresCondition: prone`, `VisionModifier: 1`, `mods/ww3mod/rules/ingame/infantry.yaml:684-686`) adds +1 to the required-vision tier, consumed in `Detectable.IsVisibleInner` (`Detectable.cs:93-116`, via `Util.ApplyAddativeModifiers` at `:99`): a prone unit must be revealed at a *higher* vision layer to be seen, i.e. it is harder to detect — same mechanism as `@Dugin`/`@InCover1-3`, mirror of `@Firing` −2 / `@Moving` −1. So prone is a real, modest stealth lever, not cosmetic. The **stronger** half remains **stopping**: a moving unit advances into the enemy's revealed cells and is seen regardless of posture; a unit that halts short stays out of them. Prone's single tier layers on top of halting — it does not substitute for it, so the correct primitive is still **"halt before you enter enemy vision,"** with prone as a real bonus on top.

On the posture mechanic itself: **deploy grants exactly the `deployed` condition** — one of four OR-clauses feeding `prone` (`ProneCondition: deployed || suppressed > 30 || !moving || critical-damage`, `infantry.yaml:261`). A stationary unit is *already* prone via the `!moving` clause, so **deploy's only unique effect is crawl** — staying prone while moving (60% speed, r20 hitshape, prone damage reduction, +1 concealment that nets 0 against the `moving −1` penalty). That crawl-on-the-advance is the natural home for an Ambush approach mode (recon §5). Because prone concealment is real, **Open Fork A's option (b)** ("add a new prone/stationary detection-reduction modifier") is largely **moot** — the mechanic already ships; the fork narrows to whether v1 *leans on* it.

> **Provenance (reclassified from [MISTAKEN] 2026-07-29):** the 2026-07-22 write trusted a detection survey that missed the `@Prone` modifier and claimed prone gave no concealment. Corrected against code by recon `WORKSPACE/recon/260728-deploy-prone.md` (2026-07-28); the engine-level statement lives in `DOCS/reference/architecture.md` under "Suppression system" (prone concealment note).
...
- Prone on the approach is a real +1-concealment lever, not cosmetic (§3.1) — and staying prone *while* attack-moving (crawl) is exactly deploy's unique effect, so the honest crawl-toward-contact is the natural implementation of this sub-behavior.
...
**A. Prone semantics.** Prone already grants +1 concealment today (§3.1), so option (b) below is largely moot — the concealment mechanic already ships. The live fork is how much v1 *leans on* it: (a) **skip explicit prone-driving in v1** and rely on the automatic `!moving` prone (cheapest — a stationary ambusher is already prone + concealed), or (c) **drive prone/crawl from the Ambush stance** so an attack-moving ambusher stays prone on the advance (deploy's unique crawl effect; recon §6 recommends this as a rider on the ambush-widening work). Which?
