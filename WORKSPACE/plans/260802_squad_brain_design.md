...
- **1b. Stop PoiOffensive shedding committed units mid-approach — LANDED, stronger form (`1fec5070`).** The design asked for a per-unit shed veto; the landed mechanism holds the whole *axis*: `PartitionHeldAxes` (`PoiOffensiveBotModule.cs:884`) pulls axes with fresh commitments out of the reshuffle entirely — held axes skip free-pool rebuild, scoring, and shed/top-up (`:637-655`), their TTL refreshed while healthy (`:946`) — and release is decided by the pure predicate `MissionCommitmentMath.ShouldReassign` (`PoiGoalGuard.cs:226`: objective invalid / danger spike ≥ `MissionDangerSpikePct` / rival beats by ≥ `MissionBetterOppMarginPct` / below half strength). Flag `MissionCommitmentEnabled` (default-off `:334`; on for `@experimental`, `ai.yaml:227`). This is trigger-1/2/3/4 of §3.3 in embryo. Blunts root cause B without the full Brain.
- **1c. Widen the believed-field score damping — SHIPPED (`f37442a3`, 2026-08-02; leg (a) completed 2026-08-04 on `auto/brain-fires-batch`).** The landed commit wired the **rival-compare** leg: `MissionCommitmentMath.QuantizeAxisScore` + `BetterOpportunityQuantized` (pure, in `PoiGoalGuard.cs`) behind a new 16-arg `ShouldReassign` overload, with the decision caller-side in `PartitionHeldAxes` (`MissionScoreQuantizeBandPct`, band pct ≤ 0 collapsing verbatim to the raw predicate — open question 1 resolved in favour of caller-side). It explicitly did **not** touch the other score consumer; the **allocation-sizing** leg landed later as `PoiOffenseMath.QuantizeSizingScores` + `AllocationScoreQuantizeBandPct`, snapping scores to the nearest band before `AllocateProportional` so near-tied axes size identically. Note the two legs use **different** transforms on purpose: floor-to-band for the rival ratio test, round-to-nearest for the allocator (flooring zeroes every axis below one band when fed to proportional sizing). Both default off; both currently held off in `@experimental` pending a priced sweep. Original description of the gap follows.
  - *(as originally designed)* Quantize scores (`QuantizeAxisScore`) so 25-tick field jitter cannot flip proportional sizes every eval, and so commitment-release comparisons operate on bands. **The landed 1b makes this urgent rather than optional:** `ShouldReassign`'s rival test compares *raw* scores against the 50% margin, and a single believed-field bucket crossing (§3.3 FIX 7: factor steps 100/60/20, 150/100/60) multiplies a score by up to 3× — clearing any percent margin and re-opening the abort/re-propose ping-pong one level up. Verified **not** in the landed code at `660a0ee2`.
- **Touched files (1c/1d):** `PoiOffensiveBotModule.cs`, `PoiGoalGuard.cs` (quantize before the rival compare), `ai.yaml` (`@experimental` flags only). **Byte-identity gate:** flags default off → `@stable`/legacy unchanged; per-profile trait instances make the single flag sufficient (no `!enable-ai-experimental` needed here). **NUnit-pinnable seams:** a pure `QuantizeAxisScore`; `MissionCommitmentMath.ShouldReassign` is already pure and pinnable. **Size of remainder:** small (~0.5 day).
...
**1d. Slider infrastructure — SHIPPED (`f37442a3`, 2026-08-02).** `Aggressiveness` (0..100, default 50) landed as an `Info` field on `PoiOffensiveBotModuleInfo` (its first consumer; migrates to the Brain in Phase 2) with the pure `PoiOffenseMath.ShiftByKnob(base, knob, slopePct)` = `base + (knob-50)·slopePct/100` threaded through one real consumer — it shifts `MissionBetterOppMarginPct` in `PartitionHeldAxes`. Ships INERT: `MissionBetterOppMarginSlopePct` defaults 0 on the engine class AND in `@experimental`, so the knob moves nothing until a sweep sets a nonzero slope. NUnit-pinned (`ShiftByKnob` neutral / zero-slope / linear-shift edges). Original description follows.

*(as originally designed)* Stand up the tunable-parameter plumbing (§2.7) even before the Brain exists: add `Aggressiveness` (and the empty `*BasePct`/`*SlopePct` scaffolding) as `Info` fields wherever the first consumer lands, and thread it through one pure function so a sweep harness can vary it per match. This is a few `Info` fields + one pure helper; it costs almost nothing and lets the owner start sweeping the aggressiveness baseline immediately (even against the Phase-1 offense tuning), rather than waiting for Phase 3. **NUnit-pinnable:** the `base ± slope` shift helper. **Size:** trivial (~half a day), bundled into Phase 1.
