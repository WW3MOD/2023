...
- **The detour threshold must sit ABOVE the territory-baseline intensity or every axis detours forever.** The Stage-C baseline stamps a low ambient danger (`BaselineIntensity`, default 5) across everything within enemy weapon reach — which is most of the approach to any objective. `GroundDangerSafeThreshold` defaults to 40 for offense (well above baseline) so only genuine defended cores (dense believed-contact kernels) trigger a flow-around; trucks use a lower 15 (a non-combatant avoids even moderate exposure). Setting the threshold at/below baseline would make the router thrash a lateral dogleg on every order.
