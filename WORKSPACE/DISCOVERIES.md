...
- **Gated to `!AlwaysTurnInPlace` (vehicles) on purpose.** Infantry are `AlwaysTurnInPlace` → `IsTurn` always false → they move through centres and have subcell fluidity already; smoothing them would fight the center-snap and interact with subcell offsets for little gain. The 45° artifact the item targets is the FullCell vehicle grid, so the gate both scopes the change and sidesteps subcell math. Benchmark is intentionally NOT byte-identical (trajectories change for all non-infantry ground units incl. bots) — determinism (integer/WDist, zero RNG) and the item-25 re-baseline are the contract, not byte-identity.
