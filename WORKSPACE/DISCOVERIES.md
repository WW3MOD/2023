...
- **The overlap guarantee is arithmetic, in `FormationRealism.LateralCap/DepthCap`.** Effective spacing is floored at `MinSlotSpacing` by the footprint cap, so two adjacent slots are ≥ `MinSlotSpacing` apart; `LateralCap` returns `< MinSlotSpacing/2` (so `2*cap < MinSlotSpacing`, pinned in `LateralCapKeepsAdjacentSlotsFromOverlapping`) and `DepthCap` returns `< min(rowSpacing/2, MinSlotSpacing)`. These are the pins that make "slots never overlap" a theorem rather than a hope.
