...
## Working on
- **AI overhaul — Phase 2 v2 experiments 1 + 2 shipped 260512.** Harness baseline 260511. **Exp #1:** `AdaptiveProductionBotModule@v2` `EvaluationInterval: 600→300` — v2 wins 65% (n=20, +15pp over 50% null). **Exp #2:** new `CaptureCoordinatorBotModule` with income-weighted target scoring, escort dispatch, defense summoning — v2 wins 60% on `tournament-capture-arena-2p` (4 neutrals in middle, n=20, +10pp lift uniform across both factions). Demo: `./tools/autotest/run-demo.sh demo-v2-capture-coordinator`. Discoveries → `DOCS/gameplay/capturing.md`: TECN captures (20 ticks), e6 has dead trait, conscript capture takes 40s. Findings: `WORKSPACE/ai/v2_experiment_002_capture_coordinator.md`. New: `DOCUMENT` recipe + `DOCS/gameplay/` folder. Next: tune another knob, expand to n=50, or jump to MapAnalyzer (foundation §4 Phase 1).
