...
**RESUMED on MSI 2026-08-06.** Both lines reconciled at `38a6d5fa` (merge of `origin/main`'s pips-zoom batch with MSI's `auto/transport-lift`); build clean, NUnit **1154/1154**. `DISCOVERIES.md` conflicted append-vs-append a third time — resolved keep-both, verified as the exact union (168 headings, zero deletions against either parent). **NOT PUSHED:** the continuous-push directive above contradicts `CLAUDE.md`'s hard rule ("NEVER push to remote — the user pushes manually"); parked as a user decision rather than resolved unilaterally. `main` is 5 ahead of origin until then.
