...
## Working on
- **Live-play fix campaign (PRIORITY)** — all FIVE River Zeta issues root-caused; 2 of 5 closed. Live board: [`STATUS.html`](STATUS.html). (5) composition **MERGED @ `9fe22a11`**, live in @experimental. (2) supply-hunt T1 **MERGED @ `7fb41816`, pushed, 1006/1006** — five rounds (aura delivery gate + hunt leash; auto-seek OFF; provider-death token-orphan fix; Carryall phantom-serving IsInWorld guard — pre-existing bug, caught by review; citation fixes). Provider fixes live in ALL profiles incl. @stable. T2 truck hunt + default flips queued, dispatch when pace allows. (1)+(3) Waves A+B implementing (`auto/waves-ab`). (4) transport wave implementing (`auto/transport-employ`) — retargeted to dedicated budget slice + filter extension (MaxActiveSquads starvation, manager-verified). Three fork records posted, proceeding on defaults (tactical ON for humans / terminal evac / forward-assemble).
