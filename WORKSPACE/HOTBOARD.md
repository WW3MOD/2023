...
## Working on
- **Live-play fix campaign (PRIORITY)** — all FIVE River Zeta issues root-caused; **3 of 5 closed**. Live board: [`STATUS.html`](STATUS.html). (5) composition **MERGED @ `9fe22a11`**, live in @experimental. (2) supply-hunt T1 **MERGED @ `7fb41816`, pushed, 1006/1006**; provider fixes live in ALL profiles incl. @stable; T2 truck hunt + default flips queued. (4) idle transports **CLOSED — MERGED @ `dadd8aee`, NUnit 1031/1031 on merged main, pushed**: TransportMissionSlots dedicated slice (root cause MaxActiveSquads starvation), use-or-evac at 900 idle ticks, demand-gated purchase; one review round caught the real blocker (Employ proxy omitted IsReadyForMission), fix `fd3bc036` + delta re-review MERGE-READY; NITs A/B/C (gate-parity notes, RiskWeightedDropSite third coupling, pins-bind-math-not-fold honesty) documented @ `5fdf9898`. Live-game validation with the user. (1)+(3) Waves A+B **LANDED @ `8fb0c855`+`d91e10f7`** on `auto/waves-ab` (958/958; 0 conflicts vs main per merge-tree) — **next lane**: manager verification, then adversarial review, one dispatch per wake. Three fork records posted, proceeding on defaults (tactical ON for humans / terminal evac / forward-assemble).
