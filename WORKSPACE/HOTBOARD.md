...
## Working on
- **Live-play fix campaign (PRIORITY)** — all FIVE River Zeta issues root-caused; three lanes implementing in parallel. Live board: [`STATUS.html`](STATUS.html). (5) `auto/composition` **MERGED @ `9fe22a11`, pushed** — live in both @experimental profiles. (2) `auto/supply-hunt` — provider-death token-orphan **blocker closed** round 3 (`b75b5703`, re-review verified, 968/968); **round 4 @ `c1028e6b`**: re-review caught a false engine claim — removal does NOT stop ITick traits (no IsInWorld/Disposed filter in `TraitDictionary.ApplyToAllTimed`), so a Carryall-carried provider kept phantom-serving from its frozen pickup position (pre-existing bug) — IsInWorld tick guard + corrected docs; manager verified all engine citations; scoped re-review in flight → merge → push. (1)+(3) Waves A+B implementing (one worker, `auto/waves-ab`). (4) transport wave implementing (`auto/transport-employ`) — employment root cause **corrected mid-flight** (manager-verified): transport missions starve behind shared `MaxActiveSquads` (:892), role filter (:1284) only gates retirement; retargeted to dedicated budget slice + filter extension. Three fork records posted, proceeding on defaults (tactical ON for humans / terminal evac / forward-assemble).
