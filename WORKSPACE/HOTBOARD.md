...
## Working on
- **AI overhaul — Stage A+B mostly shipped 260513, **B.4 blocked by IsIdle issue**.** Doctrine: `WORKSPACE/ai/doctrine.md`. **Read `WORKSPACE/ai/handoff_260513.md` to pick up.** Working: InfluenceMap + FrontlineOverlay (perception), CaptureCoordinator (income-weighted + escort), LayeredDefenceBotModule (reserve-driven slot scoring + cover seeking), capture rules cleanup (TECN-only for neutrals). NOT working empirically: MountedTransportBotModule@v2 — diagnostic logs show `carriers-candidate=0` every scan because owned carriers are never IsIdle (likely AutoTarget engaging enemies at standoff). TECN order overwriting also still reported but root cause unconfirmed. Next: per-carrier diagnostic to confirm IsIdle theory; relax IsIdle requirement or change carrier engagement stance.
