...
## Working on
- **AI overhaul — Doctrine set + Stage A queued 260512.** **New doctrine doc** `WORKSPACE/ai/doctrine.md` — real-world tactics as the basis: defence in depth (Screen/Main Line/Reserve), 3:1 concentration of force, honest fog, reserve plugs gaps. Every future AI piece evaluated against this. **Stage A next:** frontline perception — per-cell influence map + contested-band derivation + F11-toggle debug overlay. 6 verifiable TODOs in `WORKSPACE/ai/stage_a_frontline_perception.md`. Pure perception; no bot decisions change in Stage A. Earlier wins still in: v2 exp #1 AdaptiveProduction 600→300 (65% winrate), v2 exp #2 CaptureCoordinator (60% on capture-arena). Capture rules cleaned 260512: TECN-only for neutrals, soldiers take by force from enemy only.
