...
## Working on
- **Live-play fix campaign (PRIORITY)** — all FIVE River Zeta issues root-caused; round 3. Live board: [`STATUS.html`](STATUS.html). (5) `auto/composition` **MERGED @ `9fe22a11`, pushed** — live in both @experimental profiles (issue 5 closed pending live-play). (2) `auto/supply-hunt` — re-review verified rounds 1-2 all fixed (independent NUnit 965/965) but found **1 BLOCKER**: provider death orphans the permanent `replenish-soldiers` token (no death notifications on `SupplyProvider`; no source-death sweep in `ExternalCondition`; decay inert) → free perpetual reloads. Manager confirmed every premise in code; **fix round 3 dispatched** (death-notification revokes + 2 defensive revokes + seeker allocation caching) → re-review → merge → push. Waves A/B + (4) transport-employment wave (now unblocked) all dispatch at the 5h window rollover: Wave A = (1) OOA evac sweep + LC capture tier, Wave B = (3) SR-pooling fixes. Three fork records posted, proceeding on defaults (tactical ON for humans / terminal evac / forward-assemble).
