...
> ✅ **REFRESHED 2026-09-19 at `main @ 89a2e325` — the status line above is HISTORICAL; the mode is LIVE.** `MarkAsPlaceholder` has been `false` since `ebfa8828` (phase clocks in minutes, timeline read-only). The dividing wall derives on all ten maps (`ed27965d`) and River Zeta carries a hand-authored border region instead (`184680d4`); the in-game readout ships (`1aae0e0f`); hold-fire and cease-fire are gated on the game mode and Sandbox is off the dropdown (`89a2e325`). Decisions 09–22 in the same directory settle what the block above lists as owed: the ladder is per pair of sides, attributed by damage dealt, two sides only (13, 14, 15); the four lobby defaults, the timeline as primary control and the first-warheads offset (17, 18, 21); Skirmish gets a real unlock clock (22); no eviction from the wall (11). **Still open and being worked in the 2026-09-19 backlog drain (repo backlog is the live list):** hold-fire scenarios (three, each with a Skirmish discriminator); the three Escalation scenarios opting out of impact-deferred level-up; pace clocks derived from deployment math plus a stage-by-stage gameplay review; hand-authored borders for the nine remaining maps (user ruled 2026-09-19: agent authors, user reviews in play); Forward Deployment placing units inside the band on arena-tank-duel.
...
> ⚠️ **REFRESHED 2026-09-19 at `main @ ed77a8da` — NOT STARTED, and the first version of this refresh block (pushed minutes earlier at `ed77a8da`) said the opposite; it was wrong.** `git log -S ScenarioLobbyDropdown` returns `5646c71c`, which is the commit that ADDED the scenario system, not one that removed it (`-S` finds a change in either direction — read the commit). The layer is still in source, every piece of it: `engine/OpenRA.Mods.Common/Traits/World/ScenarioLobbyDropdown.cs` itself, plus `ShellmapScenario` / `ScenarioNames` / `"scenario"` filters in `OpenRA.Game/Game.cs`, `Map/Map.cs`, `Map/MapPreview.cs`, `World.cs`, `Widgets/Logic/Lobby/LobbyLogic.cs`, `LobbyOptionsLogic.cs`, `LobbyActiveChangesLogic.cs`, `MapPreviewLogic.cs`, `MapChooserLogic.cs`, and `mods/ww3mod/maps/river-zeta-ww3/scenarios.yaml` still ships. What misled the first refresh: a `grep -rl … | head -3` whose first three hits were `engine/bin`/`obj` build outputs, so the source file never appeared in the truncated listing. Sequencing from decision 08 still binds: shellmap first, deployment dropdown second, deletion last. **Lesson, same as the queue's standing hazard: a `-S` hit is not a removal, and a symbol missing from one grep is not a subsystem missing from the tree.**
