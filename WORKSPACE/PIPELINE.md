...
> ⚠️ **REFRESHED 2026-09-19 at `main @ ed77a8da` — NOT STARTED, and the first version of this refresh block (pushed minutes earlier at `ed77a8da`) said the opposite; it was wrong.** `git log -S ScenarioLobbyDropdown` returns `5646c71c`, which is the commit that ADDED the scenario system, not one that removed it (`-S` finds a change in either direction — read the commit). The layer is still in source: `ShellmapScenario` / `ScenarioNames` remain in `engine/OpenRA.Game/Game.cs`, `Map/Map.cs` and `Map/MapPreview.cs`, and `mods/ww3mod/maps/river-zeta-ww3/scenarios.yaml` still ships. Only the dropdown class name is absent from tracked source (it survives in stale `engine/bin` outputs), which is what misled the first refresh. Sequencing from decision 08 still binds: shellmap first, deployment dropdown second, deletion last. **Lesson, same as the queue's standing hazard: a `-S` hit is not a removal, and a symbol missing from one grep is not a subsystem missing from the tree.**
