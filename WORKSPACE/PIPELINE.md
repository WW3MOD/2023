...
> ✅ **REFRESHED 2026-09-19 at `main @ 89a2e325`:** the lobby scenario layer is already gone from source — `ScenarioLobbyDropdown` survives only in stale `engine/bin` outputs; removal commit `5646c71c` (`Scenario system: scenarios live inside maps, selectable via lobby dropdown`). One `scenarios.yaml` remains under `mods/ww3mod/maps/` (see `ls mods/ww3mod/maps/*/scenarios.yaml`) — whether it is still read is the next check for whoever finishes this item. Sequencing from decision 08 still binds: shellmap first, deployment dropdown second, deletion last.
