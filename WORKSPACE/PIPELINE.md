...
**R17. The developer damage overlay ships DEFAULT ON and must be switched off before release.** *(systems; created deliberately 2026-08-30, not discovered)* <!-- HITCHECK-OVERLAY-DEFAULT-ON -->
**Perceived:** a stranger's first firefight shows floating red damage numbers over every unit that takes a hit, plus `ARMOUR 54000->192` banners on anomalies. That is the audit's own definition of an "immediately-visible this-is-unfinished signal".
**This is not a defect — it is a deliberate, time-boxed debt taken on the user's ruling of 2026-08-30:** *"We could have a debug setting that reveals these in game too, and that could be made default on for now, as long as we change it before release so make notes appropriately."*
**The fix is one word:** `DebugVisualizations.DamageNumbers` (`engine/OpenRA.Game/Traits/World/DebugVisualizations.cs`) `= true` → `= false`. The in-game checkbox (*Damage Numbers*, in the debug panel next to Depth Buffer) and the `hitcheck.log` anomaly channel both stay — only the default changes. Nothing else needs touching.
**Do not delete this entry to make the list shorter.** `DebugVisualizationDefaultsTest` reads this file and asserts the marker comment above is present *if and only if* the default is still `true`, so the entry and the code cannot drift apart: removing this line while the default is on fails the build, and flipping the default without removing this line fails it too. When you flip the default, delete this entry in the same commit and the test will go green.
**Size:** one word, plus deleting this entry.
