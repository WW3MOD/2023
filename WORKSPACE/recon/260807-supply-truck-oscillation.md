...
- A unit "becomes known" purely by **being inside the 5-cell aura** at scan time. There is no request, no queue, no demand signal. `AmmoPool.NeedsResupply` exists but has only two readers — the Hunt-stance scan and a production gate (`UnitBuilderBotModule.cs:487`) — neither of which moves a truck.
  > **CORRECTION 2026-08-27: ONE reader, not two, and the paragraph's conclusion is unaffected.** `UnitBuilderBotModule.AnyFieldedUnitNeedsResupply` merely *contains the name* — its body never touches the property, computing need from `Info.Ammo` / `CurrentAmmoCount` / `Info.SupplyValue` via `ResupplyDemand.UnitNeed` (`UnitBuilderBotModule.cs:924-942`). Grep the property ACCESS (`\.NeedsResupply` minus assignments), not the string; exactly one non-assignment site survives engine-wide, `SupplyProvider.cs:622`. This line is corrected in place because it is the likely SEED of the same error in `DOCS/reference/economy.md`, which asserted "two readers" until 2026-08-27 and was itself copied into two C# comments before being caught. Fixing the downstream copies without this one just re-seeds it for whoever greps for prior art next.
