...
- **Evacuation exposure rose ~12× when evacuation was repointed to your own side — you approved the RULE, not this consequence** *(item 78, shipped `ac16f2b6` 2026-09-05; recorded here 2026-09-21 on closing the item)*. Your ruling was verbatim *"All evacuation should only happen on our own side, the one where our units spawn… Possibly that they can evacuate at any allied SR as well, on their edge, if it is closer."* That is exactly what ships. **What it costs, measured statically over the nine unit-anchored shipped maps** (`tools/evac-edge-math/`, [`audit/260921-item78-edge-arithmetic.md`](audit/260921-item78-edge-arithmetic.md)): a unit evacuating from raid range used to drive a median **9 cells** to the nearest wall and now drives **~108** home — worst case on `twin-rivers-ww3`, **2.0 → 159.8 cells**. Straight-line, so those are lower bounds. `INotifySold.Sold` fires only on arrival (`RotateToEdge.cs:517`) and the refund scales with current HP (`:511`), so **a unit killed en route banks nothing at all** — the likely real effect is that forward units stop banking refunds rather than banking them at home. **Nothing here is blocked on you and no question is being asked**; the rule is yours and it shipped. What comes back to you is the ship value once you have played it: if evacuating a damaged unit from the front now feels pointless, that is this, and it is tunable without revisiting the rule. **`@stable` inherits it** — `RotateToEdge` is the shared path for the manual Evacuate order, evacuate-when-dry, `DropsSupplyCache`, `VehicleCrew` and `EvacuateWhenUnrearmable` — so the next benchmark baseline must be re-taken knowingly.
