...
`mod.yaml:3` is still `Version: release-20230225` (the *engine* version, and the string the launcher registers). `MainMenuLogic.cs:280` and `ModInfoPanelLogic.cs:22` still hardcode **"Pre-Alpha"**. Options as posted: **`0.9 / Beta` [72] / `1.0` [66] / defer and leave "Pre-Alpha" [45]**. One line each once you pick. The branding worker was explicitly told to leave both strings untouched pending this.

**If you say nothing:** a public release ships announcing itself as Pre-Alpha, versioned as a 2023 Red Alert engine build.

### 3. Keep advertising on `master.openra.net`? — courtesy as much as dependency
There is still **no `WebServices` block anywhere in `mods/ww3mod/`** (verified by grep at `de78a1ed`), so a total conversion lists itself on upstream OpenRA's public infrastructure. Pointing `WebServices.ServerList` / `ServerAdvertise` at our own master is a `mod.yaml` block, not code. Note an adjacent decision was already taken *around* this and left it alone deliberately: `languages/en.ftl:36` records a choice **not** to reword the consent dialog to say "optimize WW3MOD", precisely because the payload really does go to `master.openra.net`.

**If you say nothing:** status quo — it keeps advertising there.
