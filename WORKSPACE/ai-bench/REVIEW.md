...
| **Last cycle** | 2026-07-19 · `2d5433a` (branch) · **first AI-side change** — delisted `$0` `logisticscenter` from v2 income tables; smoke confirms v2 now captures the nearest derrick, but S1 metric still reads 0 (metric-side, see headline) |
| **Headline** | **The AI fix WORKS; the yardstick is now the blocker.** Delisting the `$0` `logisticscenter` from v2's income tables did exactly what it should: v2's top capture target flipped to the nearest OILB (`targets 15→13`), and the hidden smoke shows v2 **captured and still owns** that derrick at match end (`buildings_killed/dead=0`; USA `assets_value` carries +200 building). **But `resources_earned` is still 0** — it reports *net* `PlayerResources.Earned`, which in the SR-budget economy never rises for a lone captured `$50` derrick (both bots read 0 before *and* after the capture; the harvester credit-path is unused). So S1 is blocked on a **metric** defect now, not v2 behavior. **Next cycle = harness fix** (gross capture-income accumulator in `BotVsBotMatchWatcher`), no re-roll. Fix committed on `ai-bench` (`2d5433aa`, not merged). Finding: `runs/260719_s1_earned_metric_finding.md`. |
...
|---|---|---|---|---|---|---|
| S1 Economy Race (5 min) | `resources_earned` | 0 (N=1 smoke) | 0 (N=1 smoke) | ≥ control ×1.15 | blocked on METRIC — AI fix DONE + verified (v2 now captures & owns the nearest OILB), but `resources_earned` (net `Earned`) is structurally blind to a lone captured derrick's gross income. Next: gross capture-income metric. | `2d5433a` (branch) |
...
- **S1 now blocked on the METRIC (found `2d5433a`, HARNESS-side):** `resources_earned`
  reports *net* `PlayerResources.Earned`, which in WW3MOD's SR-budget economy only rises
  on a net-positive periodic economy tick and never via the (unused) harvester path
  (`PlayerResources.cs:199-211`). A lone captured `$50` derrick nets 0, so `Earned` is
  **structurally blind to owned-derrick income** — both bots read 0 the whole match,
  before *and* after v2's confirmed capture. **Fix (next cycle, no AI change, no re-roll):**
  add a **gross** capture-income accumulator to `BotVsBotMatchWatcher` and repoint S1 at it;
  or confirm the lobby `passiveincome`/upkeep settings if upkeep is masking income. Still
  needs an S1 mirror twin + Normal-vs-Normal calibration afterward (SPEC §9.4). See
  `runs/260719_s1_earned_metric_finding.md`.
