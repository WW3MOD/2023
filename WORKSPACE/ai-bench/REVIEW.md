...
| **Last cycle** | 2026-07-19 18:46Z · `86aa2db` (branch) · S1 map rescope + smoke (Mode B) — new scenario GREEN, metric still 0/0 |
| **Headline** | S1 rescoped off the POI-less stub onto `tournament-s1-eco-river-zeta` (real River Zeta terrain + all 12 OILB derricks). New map boots + runs a full match cleanly; the old "0-by-construction" wall is gone. But the smoke shows a **new** blocker: neither bot captures a derrick in 5 min (v2's capture layer didn't fire in-window; Normal has none by design). S1 is now a *live, discriminating* test — the first AI cycle is "make v2 actually capture". Committed on `ai-bench`, not merged to main. |
...
|---|---|---|---|---|---|---|
| S1 Economy Race (5 min) | `resources_earned` | 0 (N=1 smoke) | 0 (N=1 smoke) | ≥ control ×1.15 | blocked — v2 doesn't capture in-window (map fixed, AI gap) | `86aa2db` (branch) |
...
- **S1 blocked on v2 capture behavior (new, from the `86aa2db` smoke):** the map
  rescope removed the "0-by-construction" wall, but v2 still earns $0 because it
  doesn't capture any of the 12 reachable derricks in 5 game-minutes. This is the
  first AI-cycle target, not a harness bug. Also needs an S1 mirror twin + a
  Normal-vs-Normal calibration batch before any v2 S1 number is trusted (SPEC §9.4).
  See the Inbox item + `runs/260719_1844__…__86aa2db.json`.
