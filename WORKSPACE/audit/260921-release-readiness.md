...
**CORRECTED 2026-09-22:** S2 was closed at `61546a51` on 08-16; the audit re-checked cites at the READER (`AttackMoveActivity.cs`), the fix landed in the WRITER (`LaneAmbushBotModule`/`AutoTarget`). Static guard: `BotOrderedMutationTest`. See DISCOVERIES 2026-09-22.
...
| **10** ✅ | ~~**Saved-game restore leak**~~ *(void 2026-09-22, `wt/savegame-facing`: the leak was already fixed at `61546a51` on 08-16 and verified green at four seeds + the human layout)* | Nothing — save and reload already does not produce a divergent world. | ~~`Activities/Move/AttackMoveActivity.cs:195-196,236,247,260`~~ — the reader, not the writer; the fix was in `LaneAmbushBotModule.cs` + `AutoTarget.cs` | the save/reload scenario **does** exist and is green (`test-savegame-resume-riverzeta`, sync-hash validated, so every `[Sync]` field and not just `Mobile.Facing`); the discipline is now pinned statically by `BotOrderedMutationTest` | **—** | no launch |
