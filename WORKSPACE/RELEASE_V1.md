...
- [T] **Supply & ammo economy overhaul (260506, plan: `WORKSPACE/plans/260506_supply_ammo_economy.md`)** — Phases 1–3 shipped:
...
- [~] **Artillery: manual click + force-attack-ground both blocked during setup-ticks countdown** — *Reported 260508. Layer 1 fixed 260509 (commit 51db91f7); auto-asserting repro at `test-arty-force-attack-during-setup` currently FAILS — exposes a Layer 2 sub-bug below.*
  - Manual click on enemy vehicle shows the **move** cursor instead of attack cursor.
  - Ctrl+click force-attack on terrain accepts the order (red waypoint shows) but the Paladin never aims/fires.
  - **Layer 1 — order-eating (FIXED)**: `AttackFollow.AttackActivity.OnLastRun` was clearing `RequestedTarget` even when a new attack activity had been queued behind us. With `QueueActivity(queued: false, …)`, the old AttackActivity ends naturally on the next tick — its `OnLastRun` fires *after* `OnResolveAttackOrder` has already pointed `RequestedTarget` at the new target. `ClearRequestedTarget()` then silently wiped the player's order. Fix: skip the clear when `NextActivity is AttackActivity`.
  - **Layer 2 — turret stalls mid-rotation (OPEN)**: with Layer 1 fixed, `RequestedTarget` correctly stays on the new target, but the turret only rotates partway and stalls. Trace shows `localYaw` advancing 779 → 629 in one second, then stuck (`desired=583`, modifiers all >0, `desiredDirection` non-zero). Likely cause: `Turreted.Tick`'s realign-to-`InitialFacing` path activating during the rotation window when `IsAiming=False`, fighting `FaceTarget` for `desiredDirection` / `realignDesired` ownership. Touches: `Turreted.cs:191-216` (Tick/realign), `AttackTurreted.cs:36-48` (CanAttack short-circuit). Probably needs the realign delay to start counting only when there's *no* aim request, not when the unit just isn't ready yet.
...
- [x] **Parallel queues build paused units** — *Verified false-alarm 260509 via `test-parallel-queue-pause`*: queued an `e3.america` on USA's parallel Infantry queue, paused it, waited 250 ticks (10s, much longer than build time) — zero spawned. Then unpaused, waited another 250 ticks — unit DID spawn. Confirms pause is honored at all three layers (queue tick, item tick, pause-call) and the queue is actually capable of producing. Tracker entry was stale; no fix needed.
