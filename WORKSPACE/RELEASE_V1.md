...
- [v1.1] Airstrike support powers (A-10, Su-25) — hidden for v1. `AirstrikePower` + lobby option commented out in `player.yaml`/`world.yaml`; A10/FROG actor defs left orphan. **CORRECTED 2026-09-04: "re-enable by uncommenting" is WRONG and was wrong in the expensive direction.** Uncommenting ships a working Su-25 power and a NATO power that cannot fire a shot: the strafe aim point is a Terrain target, `Armament.CanFire` refuses a weapon not valid against the cell's `Ground` type (`Armament.cs:402`), and neither of `A10.Airstrike`'s weapons lists `Ground` — `^30mm` (`weapons-ballistics.yaml:579`) and `Hellfire` (`weapons-missiles.yaml:243`). The Su-25's `RocketPods` does. Known independently at `bugs/discovered.md:127`, established by `38564a78`. So this needs a WEAPON fix before any balance pass. Separately, 13 of the 16 commented blocks are RA corpses referencing six actors that do not exist, nine of them consuming a condition `CheckConditions.cs:75` treats as a hard lint error — those are deletions, not re-enables. Full costing: `WORKSPACE/recon/powers-and-preloaded-transports.md`
