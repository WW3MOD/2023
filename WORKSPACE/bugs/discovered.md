...
- [2026-08-30] [question, not confirmed a bug] **The AT specialist's guided ATGM penetrates less than the rifleman's disposable rocket.** `ATGM` is `Damage: 10000, Penetration: 100` (`weapons-missiles.yaml:25-27`); `RPG` is `Damage: 6000, Penetration: 500` (`weapons-ballistics.yaml:516`). Against `abrams` `Armor.Thickness: 700` (`vehicles-america.yaml:499`), `ApplyPenetration` (`DamageWarhead.cs:127-133`) gives the ATGM `10000 × 100/700 = 1428` and the RPG `6000 × 500/700 = 4285` — so the 300-cost dedicated anti-tank specialist lands **a third** of what the 100-cost rifleman's throwaway does, against the target he exists to kill. Against `t90` (`Thickness: 280`) it is 3571 vs 6000. **Not filed as a confirmed bug** — I did not measure it and 100 may be deliberate — but it is the same shape as the recorded "192 of 24000" penetration incident, and it is invisible in every current tooltip. (found while working on: tooltip weapon-section mockups)
