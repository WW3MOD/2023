...
the spec. A one-line deletion is the whole fix if someone wants it; there is nothing to preserve. DIAGNOSED NOT FIXED — `Linux (mono)` CI has not compiled since 2026-08-11: `Convert.ToHexString` is net5.0+ and the mono lane targets netstandard2.1 (found while: CI reporting-integrity audit, branch `wt/ci-integrity`, `main @ 8656bd3c`)
