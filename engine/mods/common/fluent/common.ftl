...
notification-no-port-forward-discovery-disabled = Server port is not accessible from the internet. Automatic port forwarding is switched off — enable UPnP/NAT-PMP discovery in Settings, Advanced, and restart the game, or forward the port on your router by hand.
notification-no-port-forward-no-device = Server port is not accessible from the internet. No UPnP/NAT-PMP router answered, so the port has to be forwarded on the router by hand.
notification-no-port-forward-rejected = Server port is not accessible from the internet. Your router refused the automatic port mapping — an existing manual port-forward rule on the same external port is the usual cause.
notification-no-port-forward-upstream = Server port is not accessible from the internet even though your router accepted the port mapping. The block is further upstream: double NAT, carrier-grade NAT, or a firewall.
notification-no-port-forward-carrier-grade-nat = Server port cannot be opened: your internet provider places you behind carrier-grade NAT ({ $address }). No router setting can fix this — hosting needs a public IP address from your provider.
notification-no-port-forward-local-address = This machine is listening on { $endpoint }. Any port-forward rule on your router must point at that exact address.
