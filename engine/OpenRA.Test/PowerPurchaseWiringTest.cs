...
namespace OpenRA.Test
{
	...
	public class PowerPurchaseWiringTest
	{
		...
		public void TheUnlockClockIsRegisteredAndItsOptionsAreWellFormed()
		{
			...
			// THE FOUR TIER CHECKBOXES REPLACED THE `nuclear-highest-yield` CAP DROPDOWN (decision 02),
			// so the out-of-set hazard above cannot arise for them -- a LobbyBooleanOption's value set
			// is {True, False} and its default is a bool. What CAN still go wrong is the pairing: the
			// ids, the labels and the defaults are three parallel lists, and the options are generated
			// by index, so a list that grew on one side only would silently shift every tier by one.
			var ids = OpenRA.Mods.Common.Traits.NuclearUnlockClockInfo.PurchasableOptionIds;
			Assert.That(ids, Is.EqualTo(new[] { "nuke-1kt", "nuke-20kt", "nuke-50kt", "nuke-100kt" }),
				"the tier option ids are wire-visible state: renaming or reordering one silently " +
				"discards every stored value set to it");
			Assert.That(info.PurchasableDefaults().Count, Is.EqualTo(ids.Length),
				"a tier gained an id without a default, or the reverse; the options are generated by index");

			// ALL FOUR DEFAULT ON, which is what makes an untouched lobby behave exactly as the old
			// cap default did -- everything up to 100 kt on sale, game-enders never.
			Assert.That(info.PurchasableDefaults(), Is.All.True,
				"a yield tier now defaults to un-purchasable; an untouched Skirmish lobby has changed");

			// One per PURCHASABLE band and no more. A fifth would be a game-ender box, which decision
			// 17.3 refuses outright and NuclearUnlockSchedule.HighestPurchasableRung blocks anyway.
			Assert.That(ids.Length,
				Is.EqualTo(OpenRA.Mods.Common.Traits.NuclearUnlockSchedule.HighestPurchasableRung
					- OpenRA.Mods.Common.Traits.NuclearUnlockSchedule.LowestRung + 1),
				"the tier checkboxes no longer cover exactly the purchasable bands");
			...
			// And the game-ender band is not offered at all -- decision 17.3, no host override. The
			// cap dropdown that used to be checked here is gone (decision 02); what replaced it is
			// four checkboxes, so the assertion is that NONE of them names that band.
			Assert.That(ids, Has.No.Member("nuke-gameender"),
				"a game-ender purchase checkbox exists; decision 17.3 says a host cannot reach that " +
				"band in Skirmish at all");
		}
	}
}
