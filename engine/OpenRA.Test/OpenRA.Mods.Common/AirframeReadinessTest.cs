...
namespace OpenRA.Test
{
	...
	public class AirframeReadinessTest
	{
		...
		const bool Aircraft = true;
		const bool GroundUnit = false;
		const bool ReservablePadPresent = true;
		const bool NoReservablePad = false;
		const bool DockHostPresent = true;
		const bool NoDockHost = false;

		/// <summary>
		/// The dock is the only host kind a ground unit has, so it must count for one.
		/// </summary>
		[Test]
		public void AGroundUnitIsHostedByADock()
		{
			Assert.That(
				AirframeReadiness.CountsAsRearmHost(GroundUnit, NoReservablePad, DockHostPresent), Is.True,
				"A ground unit drives to a RearmsUnits/SupplyProvider dock under AmmoPool.AutoRearm. " +
				"Refusing to count it would strand every vehicle and rifleman in the mod.");
		}

		/// <summary>
		/// <para>THE REGRESSION THIS FIXTURE EXISTS FOR. Pointing the seven aircraft at `logisticscenter`
		/// makes AmmoPool.ChooseResupplier return a host for them — it filters on RearmActors
		/// membership plus a SupplyProvider/RearmsUnits trait, and never asks whether the caller can
		/// reach it. No aircraft ever can: all three AutoRearm entry points refuse AircraftInfo, and
		/// Resupply's approach block is guarded on `aircraft == null`.</para>
		///
		/// <para>If the dock term counted, the airframe would read as hosted and take the strict
		/// restore-first bars — full ammo to launch, recovery bar to re-engage — with nothing able to
		/// satisfy them. That is the benched-for-the-match defect the rest of this fixture pins,
		/// re-entered through the other term.</para>
		/// </summary>
		[Test]
		public void AnAircraftIsNotHostedByADockItCanNeverBeSentTo()
		{
			Assert.That(
				AirframeReadiness.CountsAsRearmHost(Aircraft, NoReservablePad, DockHostPresent), Is.False,
				"An aircraft was reported as hosted by a ground DOCK. Nothing dispatches an aircraft to " +
				"one (AutoRearmIfAllEmpty / AutoRearmIfAnyNotFull / the bot sweep candidate filter all " +
				"refuse AircraftInfo) and Resupply will not carry it there (its approach is guarded on " +
				"`aircraft == null`). Counting it flips the airframe onto the strict restore-first bars " +
				"with no way to satisfy them — the unsatisfiable-gate defect, re-entered.");
		}

		/// <summary>
		/// The pad is what ReturnToBase selects on, and it is the ONLY route an aircraft has to a host.
		/// A logistics center carrying Reservable is therefore a real aircraft host; one without it is
		/// not, however many supply traits it has.
		/// </summary>
		[Test]
		public void AReservablePadHostsEitherKind()
		{
			Assert.That(
				AirframeReadiness.CountsAsRearmHost(Aircraft, ReservablePadPresent, NoDockHost), Is.True,
				"ReturnToBase.ChooseResupplier filters on Reservable and flies the aircraft in, so a " +
				"Reservable host is the one kind an aircraft can actually reach.");

			Assert.That(
				AirframeReadiness.CountsAsRearmHost(GroundUnit, ReservablePadPresent, NoDockHost), Is.True);
		}

		/// <summary>
		/// No host of either kind is the shipped state on the seven maps that have no logistics center,
		/// and it must stay reachable — it is what selects the one-way-resource bars.
		/// </summary>
		[Test]
		public void NeitherHostKindMeansUnhosted()
		{
			Assert.That(AirframeReadiness.CountsAsRearmHost(Aircraft, NoReservablePad, NoDockHost), Is.False);
			Assert.That(AirframeReadiness.CountsAsRearmHost(GroundUnit, NoReservablePad, NoDockHost), Is.False);
		}
	}
}
