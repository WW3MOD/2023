...
namespace OpenRA.Mods.Common.Widgets.Logic
{
	public class ModContentLogic : ChromeLogic
	{
		...
		public ModContentLogic(ModData modData, Widget widget, Manifest mod, ModContent content, Action onCancel)
		{
			...
			var sourceYaml = ModContentPromptLogic.LoadYamlFromModPackage(mod, content.Sources);
			foreach (var s in sourceYaml)
				sources.Add(s.Key, new ModContent.ModSource(s.Value));

			var downloadYaml = ModContentPromptLogic.LoadYamlFromModPackage(mod, content.Downloads);
			...
			var headerLines = !string.IsNullOrEmpty(content.HeaderMessage)
				? content.HeaderMessage.Replace("\\n", "\n").Split('\n')
				: Array.Empty<string>();
			var headerHeight = 0;
