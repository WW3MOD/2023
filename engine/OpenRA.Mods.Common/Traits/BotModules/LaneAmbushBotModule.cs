...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class LaneAmbushBotModuleInfo : ConditionalTraitInfo
	{
		...
		[Desc("ARMY-SHARE RESERVE: refuse to recruit a unit while doing so would leave PoiOffensiveBotModule's",
			...
			"published by PoiOffensiveBotModule.TryGetFreePoolSnapshot and the floor from its",
			"EffectiveFreePoolMinAdvanceUnits — the same three terms ForwardStagingMath.FreePoolMayAdvance is",
			"decided on, so the waivers agree. A live axis waives it (the pool is no longer the whole army), and",
			"so does a floor of 0. AIMED AT THE OPENING for exactly that reason: a large pool clears the floor",
			"with room to spare and the lane recruits as before.",
	...
	public class LaneAmbushBotModule : ConditionalTrait<LaneAmbushBotModuleInfo>, IBotTick
	{
		...
		void Reevaluate(IBot bot)
		{
			...
			var allowance = ResolveReserveAllowance(out var reserveFree, out var reserveMin, out var reserveAxes);
			...
			// ARMY-SHARE RESERVE (item 86) — DIAGNOSTIC. Printed only when the reserve actually BOUND, i.e. it
			// allowed fewer units than the lane had free to take; an unbounded or non-biting eval prints nothing,
			// so the line's presence IS the decision. Styled on [exp-ledger]: one line per eval carrying the
			// terms the verdict was reached on, so a run can be read without re-deriving them. `taken` counts
			// recruits directly rather than diffing the pool, which the shed-surplus path above also GROWS, and
			// `lanes` is the shape the decision produced (lanes holding units) — at the
			// opening this reads `lanes=0 free=3`, the whole army still with offense.
			if (reserveStartAllowance != int.MaxValue && reserveStartAllowance < freeAtFill)
			{
				var manned = 0;
		...
		int ResolveReserveAllowance(out int offenseFree, out int offenseMin, out bool offenseAxisLive)
		{
			...
			if (offensive == null)
				return AmbushLaneMath.ReserveAllowance(true, false, false, 0, 0, false);
			...
			return AmbushLaneMath.ReserveAllowance(
				true, true, snapshotValid, offenseFree, offenseMin, offenseAxisLive);
		}
	...
	public static class AmbushLaneMath
	{
		...
		/// them, posted them PostFractionPct of the way to the enemy beachhead, and the tank walked to 20,14 and
		/// died there at t585 while <c>[exp-staging] hold-under-min pool=1 min=2</c> says offense was ITSELF
		/// refusing to advance the one unit it had left. Both modules behaved correctly in isolation; between
		/// them the army was split below the threshold either half needed.</para>
		///
		/// <para>THE TERMS ARE THE OFFENSE'S OWN, not a re-derivation. <paramref name="offenseFreeCount"/> is
		/// PoiOffensiveBotModule's published free-pool count, <paramref name="offenseMinAdvanceUnits"/> its
		/// EffectiveFreePoolMinAdvanceUnits, and <paramref name="offenseAxisLive"/> the axis term — the same
		/// three values <see cref="ForwardStagingMath.FreePoolMayAdvance"/> is decided on, so the waivers match:
		/// a floor of 0 is a floor nobody applies, and a live axis means the pool is no longer the whole
		/// army.</para>
		///
		...
		/// would breach the floor is unknowable — the lane takes nothing rather than guess. That state is not
		/// reachable in the shipped profiles (offense's stagger puts its first eval inside [0, interval) and the
		/// lane's is at interval), and it self-clears at offense's next eval either way.</para>
		///
		...
		public static int ReserveAllowance(bool enabled, bool offenseResolved, bool snapshotValid,
			int offenseFreeCount, int offenseMinAdvanceUnits, bool offenseAxisLive)
		{
			// Off, or there is no offensive module on this player to reserve anything FOR.
			if (!enabled || !offenseResolved)
				return int.MaxValue;
			...
			// knowable without a pool count, and must not fall into the fail-toward-offense branch below.
			...
			// FreePoolMayAdvance waives the floor whenever an axis is live, so there is nothing to protect.
			if (offenseAxisLive)
				return int.MaxValue;
