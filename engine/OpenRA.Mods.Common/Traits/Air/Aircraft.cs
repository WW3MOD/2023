...
namespace OpenRA.Mods.Common.Traits
{
		...
		IActorPreviewInitModifier, IDeathActorInitModifier, IIssueDeployOrder, IIssueOrder, IResolveOrder, IOrderVoice, IAirborneVisibility
	{
		...
		public void AdjustMomentum(WVec desiredMove)
		{
			...
			if (targetSpeed > 0)
			{
				...
				var newDir = currentDir == WVec.Zero ? desiredDir :
					(currentDir + (desiredDir - currentDir) * GetAcceleration / 32);
		...
		public int MovementSpeed => !IsTraitDisabled && !IsTraitPaused ? Util.ApplyPercentageModifiers(Info.MaxSpeed, speedModifiers) : 0;
		public int IdleMovementSpeed => Info.MinSpeed < 0 ? MovementSpeed :
			!IsTraitDisabled && !IsTraitPaused ? Util.ApplyPercentageModifiers(Info.MinSpeed, speedModifiers) : 0;
